...
enum class ApplicationStates { Startup, Loading, Menu, Running, Paused, GameOver, Quitting };
...
class MainMenuScreen : public Screen {
   public:
    void Draw() override {
        ...
        DrawText(CopyrightString, GetScreenWidth() - 2 - MeasureText(CopyrightString, 10), GetScreenHeight() - 10, 10,
                 GRAY);

        // play button
        if (CenteredButton(GetScreenHeight() / 4, "Play")) StartGame();

        // options button
        CenteredButton(GetScreenHeight() / 2, "Options");

        // quit button
        if (CenteredButton(GetScreenHeight() - (GetScreenHeight() / 4), "Quit")) QuitApplication();
    }
};
...
class GameOverScreen : public Screen {
   public:
    void Draw() override {
        ...
        // main menu button
        if (CenteredButton(GetScreenHeight() / 2, "Main Menu")) GoToMainMenu();
...
void SetupWindow() {
    ...
    Image icon = LoadImage("assets/icons/Icon.6_98.png");
...
void LoadComplete() {
    ...
    LoadMap("assets/maps/menu_map.tmx");
}
...
void GoToMainMenu() {
    ...
    if (ApplicationState == ApplicationStates::Running || ApplicationState == ApplicationStates::Paused) QuitGame();

    // start our background music again
    StartBGM("assets/sounds/Flowing Rocks.ogg");
...
void UpdateMainMenu() {
    if (IsKeyPressed(KEY_ESCAPE)) QuitApplication();
}
...
void PauseGame() { ApplicationState = ApplicationStates::Paused; }
...
void QuitApplication() { ApplicationState = ApplicationStates::Quitting; }
