...
void SetSpriteOrigin(int spriteId, int x, int y) {
    if (spriteId < 0 || spriteId >= int(Sprites.size())) return;
...
void DrawSprite(int spriteId, float x, float y, float rotation, float scale, Color tint, uint8_t flip) {
    ...
    if (flip & SpriteFlipDiagonal) rotation -= 90;
    if (flip & SpriteFlipX) source.width *= -1;
    if (flip & SpriteFlipY) source.height *= -1;

    Rectangle destination = {x, y, sprite.SourceRect.width * scale, sprite.SourceRect.height * scale};

    if (flip & SpriteFlipDiagonal) destination.y += destination.height;

    DrawTexturePro(GetTexture(sprite.TextureId), source, destination, Vector2Scale(sprite.Origin, scale), rotation,
                   tint);
}
...
void FillRectWithSprite(int spriteId, const Rectangle& rect, Color tint, uint8_t flip) {
    ...
    if (flip && SpriteFlipDiagonal == 0) rotation += 90;
    if (flip && SpriteFlipX == 0) source.width *= -1;
    if (flip && SpriteFlipY == 0) source.height *= -1;
